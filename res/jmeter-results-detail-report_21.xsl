	...
	<html>
		<head>
			...
			<style type="text/css">
				...
				h3 {
					margin-bottom: 0.5em; color:red; font: bold 115% verdana,arial,helvetica
				}
				.Failure {
			...
			<script language="JavaScript"><![CDATA[
			   ...
			   {
			      if(document.getElementById(details_id+"_image").innerText === "展开")
			      {
			         document.getElementById(details_id+"_image").innerText = "收起";
			         expand(details_id);
			      }
			      else
			      {
			         document.getElementById(details_id+"_image").innerText = "展开";
			         collapse(details_id);
			      } 
                           }
			]]></script>
...
<xsl:template name="pageHeader">
	...
	<table width="100%">
		<tr>
			<td align="right" style="font-size:100%">报告时间: <xsl:value-of select="$dateReport" /></td>
			...
			<xsl:if test="$failureCount > 0">
				...
				<xsl:for-each select="/testResults/*[@lb = current()/@lb][attribute::s='false']">
					<tr>
						...
						<!--td><xsl:value-of select="requestData" /></td-->
						<!--xsl:if test="$showData = 'y'"-->
			...
			<tr valign="top">
				...
				<td align="center">
				   <a href="">
				      ...
				      <xsl:attribute name="id"><xsl:text/>page_details_<xsl:value-of select="position()" />_image</xsl:attribute>展开		      
				   </a>
				</td>
			</tr>
